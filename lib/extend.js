 ...
 * @Last Modified time: 2019-12-02 22:40:05
 */
"use strict";
...
function setex (key, time, value) {
    if (!key || !time || !value || (typeof time !== "number")) {
        return Promise.reject("请检查setex传参");
    }
    return new Promise((resolve, reject) => {
        if (this[sock].destroyed) {
            reject({ message: "sock已被销毁,请检查redis服务是否开启", info: "" });
        ...
        this[sock].write(command.setex(...arguments), err => {
            if (err) {
                reject({ message: "sock底层发送命令失败", info: err });
                return;
            }
...
function hmset(...args){
    return new Promise((resolve, reject)=>{
        args = Array.prototype.slice.call(args);
        if (/[\u4e00-\u9fa5]/.test(args[0])) {
            reject({message: "hmset key 不能为中文", info: ""});
        }
        ...
        for (let i = 1; i < args.length; i+=2) {
            if ( !/^[^\u4e00-\u9fa5]+$/.test(args[i]) ) {
                reject({ message: "hmset 字段Key不能为中文", info: "" });
                return;
            }
        }
        if (this[sock].destroyed) {
...
function hdel (hash, field) {
    return new Promise((resolve, reject) => {
        ...
        this[sock].write(command.hdel(...arguments), err => {
            if (err) {
...
function hmget (...args) {
    return new Promise((resolve, reject)=>{
        if (/[\u4e00-\u9fa5]/.test(args[0])) {
            reject({message: "hmget key 不能为中文", info: ""});
        }
        for (let i = 1; i < args.length; i++){
            if (!/^[^\u4e00-\u9fa5]+$/.test(args[i])){
                reject({ message: "hmget 字段key不能为中文", info: "" });
                return;
            }
        }
        if (this[sock].destroyed) {
