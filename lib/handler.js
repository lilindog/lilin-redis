...
function handler(chunk)
{
    ...
    const action = ()=>
    {
        ...
        if(/^\$[1-9]\d+\r\n/.test(this[chunks]))
        {
            ...
            //正常
            let 
            num = Number( this[chunks].match(/^\$\d+\r\n/)[0].replace(/[^0-9]/g, "") ),
            reg = new RegExp(`^\\$\\d+\\r\\n(.|\\n|\\s){${num}}\\r\\n`);
            if(reg.test(this[chunks]))
            {
                this[callbacks][0] && f1()("$", this[chunks].match(reg)[0].replace(/(\$[^(\r\n)]+\r\n)|(\r\n)/g, ""));
                this[chunks] = this[chunks].replace(reg, "");
                action();
            }
        }
